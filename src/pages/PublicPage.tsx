...
import { Calendar as CalendarIcon, List, LayoutGrid, TrendingUp, Instagram } from 'lucide-react';
...
function StatsPanel({ pieces, currentDate }: { pieces: ContentPiece[], currentDate: Date }) {
  ...
  const goodPerformances = monthPieces.filter(p => p.good_performance).length;

  const byFormat = Object.entries(
    monthPieces.reduce((acc, p) => {
  ...
  return (
    <div className="mb-8">
      <p className="text-xs text-gray-400 font-medium uppercase tracking-wide text-center mb-3">
        Estadísticas — {monthNames[month]} {year}
      </p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow p-5 flex flex-col gap-1">
          ...
          {total > 0 && (
            <p className="text-xs text-gray-400">{Math.round((goodPerformances / total) * 100)}% del mes</p>
          )}
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-pink-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {!loading && <StatsPanel pieces={pieces} currentDate={currentDate} />}
